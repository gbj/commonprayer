...
impl Component for Auth {
    fn view(&self) -> Host {
        let avatar = self.user.as_ref().and_then(|user| user.photo_url.as_ref()).map(|src| view! {
            <img src={src} alt={self.user.as_ref().unwrap().display_name.clone().unwrap_or_default()}/>
        }).unwrap_or_else(|| view! {
