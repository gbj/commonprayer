...
pub fn global_search<L: Library>(
    ...
    let hymns = hymnals.into_iter().flat_map(|hymnal| {
        hymnal.hymns.iter().filter_map(|hymn| {
            hymn.search_in(cx, raw, &q, &q_date, &q_citation, language, None, None)
        })
    });

    // Search for matching holy days
    let holy_days = LFF2018_FEASTS.iter().filter_map(|(_, feast, ..)| {
        feast.search_in(cx, raw, &q, &q_date, &q_citation, language, None, None)
    });
    ...
    let documents = contents
        ...
        .flat_map(|(slug_path, c)| {
            search_contents(cx, c, slug_path, raw, &q, &q_date, &q_citation, language)
        });

    // Search within Psalter
    let psalms = BCP1979_PSALTER.psalms.iter().flat_map(|(_, psalm)| {
        psalm.search_in(cx, raw, &q, &q_date, &q_citation, language, None, None)
    });
...
fn fuzzy_string_match<'a>(text: &'a str, raw_q: &str, q: &Regex) -> PossibleMatch<'a> {
    ...
    match captures {
        ...
        Some(captures) => {
            ...
            let score = (strsim::jaro_winkler(raw_q, matched_str) * 1000.0) as i32;
    ...
    fn search_in(
        ...
        match self {
            ...
            Contents::Document(document) => {
                document.search_in(cx, raw, q, q_date, q_citation, language, slug_path, path)
            }
                ...
                std::iter::Iterator::intersperse_with(
                    slug_path
                        ...
                        .map(|slug| match slug {
                            ...
                            Slug::Canticle(id) => t_with_args(
                                "daily-readings-canticle",
                                HashMap::from([("number", id.to_string())]),
                            ),
...
impl Searchable for Psalm {
    fn search_in(
        ...
        let number_raw = format!("Psalm {}", self.number);
